...
Session Context laden (Redis/Mongo)

Query-Preprocessing (kein DB-Zugriff)

Vektor-Search auf Chunks (pgvector)

Dokumente nachladen (Mongo/JSONB)

Produktmetadaten prüfen (PG)

Berechtigungen prüfen (PG)

Antwort-Komponenten cachen (Redis)

Logging (Timescale/PG)
...
Ihr seht an einem ganz konkreten Beispiel, wie viele Einzelschritte ein einziger Support-Request tatsächlich hat – und warum dabei mehrere Datenbanken beteiligt sind.
...
Prüfen, ob der Request zulässig ist (Rate-Limits, Missbrauch).

Den aktuellen Chat-Kontext kennen (vorherige Nachrichten im Verlauf).

Relevante Dokumentstellen finden (Produktbeschreibung, Anleitung, Troubleshooting-Artikel).

Prüfen, ob der Support-Agent für diesen Kunden / diese Daten berechtigt ist.

Die Antwort des LLM loggen (für Monitoring und Audit).
...
pgvector: 1 ANN-Query

Mongo: 1–2 Queries (3–8 Chunks)

Postgres: 1–2 Queries (Metadaten + Permission)

Redis: SETEX (Cache)

Mongo: updateOne oder insertOne (Chat)
...
Ziel dieses Abschnitts:
Ihr versteht, warum „210 DB-Operations pro Sekunde“ nicht einfach bedeutungslos sind, obwohl ihr lokal 10.000 Inserts/s gemessen habt.
...
Exzellent:
„Bei 70 parallelen Chats × 1 RAG-Query alle 6s entstehen ~12 Queries pro Sekunde (70/6 ≈ 11.7). Dazu kommen 2–3 weitere Queries pro Sekunde durch Parallelaktionen wie Ticketupdates. Wir rechnen mit 15 RAG-Requests pro Sekundeunter Normalbetrieb und bis zu 40 unter Lastspitzen (Sale-Events, Black Friday).“
