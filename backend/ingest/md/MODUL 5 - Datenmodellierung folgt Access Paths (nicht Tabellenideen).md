# MODUL 5 — Datenmodellierung folgt Access Paths (nicht Tabellenideen)
...
- Modul 4 = verstehen, welche Pfade kritisch sind (Analyse der Zugriffswege und Latenzen)
- Modul 5 = das Modell so designen, dass diese Pfade optimal laufen (Access-Path-basierte Datenmodellierung)
...
1. Finde alle Lieferereignisse der letzten 30 Tage.
2. Berechne die Lieferdauer je Ereignis.
3. Sortiere alle Lieferdauern.
4. Nimm den mittleren Wert → Median.
...
❌ **EMBED (falsch)** — Beispiel für eine schlechte Modellierung:
...
✔️ **REFERENCE (richtig)** — Eine Lieferung = ein einzelnes Dokument/Objekt.
...
#### 2.2 Big Document vs. Small Document

❌ **Big Document (falsch)** — Beispiel:
...
✔️ **Small Document (richtig)** — Small Documents:
...
#### 2.3 Precompute vs. Compute-on-Read

❌ **Compute-on-Read (falsch)** — Jede Anfrage müsste:
...
✔️ **Precompute (richtig)** — Median wird zyklisch im Hintergrund berechnet.
