...
2. Modellierungsentscheidungen
4.1 EMBED vs REFERENCE
❌ EMBED (falsch)
Beispiel (schlechte Modellierung):
...
✔️ REFERENCE (richtig)
Chunk-Dokument bleibt klein, Embedding ist separat gespeichert.
...
Ein Chunk = ein Dokument
Typisch 1–3 KB Text + Metadaten.
...
Metadaten erst beim Lesevorgang extrahieren (z. B. Überschriften, Positionen).
...
Critically path: schneller Lookup + sicheres Update.
...
4.2 Big Document vs Small Document
❌ Big Document (falsch)
Alles in einer „Kunden-Mega-Zeile“ führt zu:
...
❌ Precompute (hier nicht sinnvoll)
Kein Nutzen, verursacht nur Komplexität.
...
4.1 EMBED vs REFERENCE
❌ EMBED (falsch)
Das Gesamtdokument enthält alle Chunks als Liste.
...
✔️ REFERENCE (richtig)
Chunk als eigenes Dokument.

4.2 Big Document vs Small Document
❌ Big Document (falsch)
Gesamtes Manual in einem Objekt:
→ 100–500 KB
...
❌ Compute-on-Read
Aus Text extrahieren → teuer → nicht im kritischen Pfad.
...
❌ REFERENCE (falsch)
Einzelne Nachrichten als eigene Dokumente → viele Roundtrips.

4.2 Big Document vs Small Document
❌ Zu großes Dokument
Wenn messages-Array > 1–2 MB:
...
✔️ EMBED (richtig)
Eine Konfiguration ist eine atomare Einheit → Änderungen immer als Ganzes.
...
4.2 Big Document vs Small Document
❌ Small Documents (falsch)
Warum nicht 20 kleine Dokumente?
...
✔️ Big Document (richtig)
Eine Version = 1 großes JSON (100–500 KB)
→ ideal für:
...
❌ Compute-on-Read
Gesamtes Dokument live zu diffen → viel zu teuer.
